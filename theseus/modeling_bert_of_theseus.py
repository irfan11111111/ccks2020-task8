...
def load_tf_weights_in_mobilebert(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        if "cls" in name:
            print(name)
        if any(
                n in ["adam_v", "adam_m", "AdamWeightDecayOptimizer", "AdamWeightDecayOptimizer_1", "global_step","teacher"]
                for n in name
        ):
            # logger.info("Skipping {}".format("/".join(name)))
            continue
        if str(name[0]).startswith("hidden_distill"):
            continue
        pointer = model
        # print(name)
...
class MobileBertEncoder(nn.Module):
    def __init__(self, config, scc_n_layer=12):
        ...
        self.layer = nn.ModuleList([MobileBertLayer(config) for _ in range(self.prd_n_layer)])
        self.scc_layer = nn.ModuleList([MobileBertLayer(config) for _ in range(self.scc_n_layer)])
        # self.output_attentions = config.output_attentions
        # self.output_hidden_states = config.output_hidden_states
...
class MobileBertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.vocab_size, config.hidden_size - config.embedding_size, bias=True)
