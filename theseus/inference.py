        ...
        for i, batch_data in enumerate(tqdm(test_dataloader)):
            ...
            for pred, text, file_name in zip(best_path, texts, file_names):
                ...
                for i in range(len(pred_entities)):
                    text=pred_entities[i]['entity_text']
                    if text in checkset:
                        dellist.append(i)
                    else:
                        checkset.add(text)
...
if __name__ == '__main__':
    # run('configs/base_config.json', 'saved/ner/models/seq_label/1227_105459/model_best.pth')
    run('configs/base_config.json', './saved_predecessor/pytorch_model.bin')
