...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if from_tf:
            ...
            start_prefix = ""
            model_to_load = model
            has_prefix_module = any(s.startswith("bert") for s in state_dict.keys())
            if not hasattr(model, "bert") and has_prefix_module:
                start_prefix = "bert" + "."
            if hasattr(model, "bert") and not has_prefix_module:
            ...
            if model.__class__.__name__ != model_to_load.__class__.__name__:
                ...
                head_model_state_dict_without_base_prefix = [
                    key.split("bert" + ".")[-1] for key in model.state_dict().keys()
                ]
