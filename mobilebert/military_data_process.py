...
class MilitaryDataSet(NLPDataSet):
    ...
    def _load_dataset(self):
        ...
        for train_file in train_file_list:
            with open(os.path.join(self.data_dir, train_file), 'r', encoding='gbk') as f:
                ...
                # print(len(inputs["input_ids"]),inputs)
    ...
    def collate_fn(self, datas):
        ...
        for data in datas:
            text = data.text
            # print(len(text),text)
            # print(len(data.inputs.data['input_ids'][:-1]),data.inputs.data['input_ids'][:-1])
            text_lengths.append(data.text_len)
            texts.append(text)
            # print(len(data.inputs['input_ids']),data.inputs['input_ids'])
            # for i in range(len(data.inputs["input_ids"])):
            #     print(i,data.inputs["input_ids"][i+1],text[i])
            # 把 sep 删除掉
        ...
        # print(text_lengths)
        # for it in bert_masks:
        #     print(len(it))
        # print(np.array(text_token_ids))
    ...
    def _load_testset(self):
        ...
        """
        examples = []
        with open(os.path.join(self.data_dir,'validate_data.json'),'r',encoding="utf-8") as f:
